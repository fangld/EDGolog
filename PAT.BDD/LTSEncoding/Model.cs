...
namespace PAT.Common.Classes.SemanticModels.LTS.BDD
{
    ...
    {   
        ...
        public const string TEMP_TRANSITION = "temp_event#";
        ...
        /// </summary>
        public List<CUDDNode> variableRanges = new List<CUDDNode>();
        ...
        public Model()
        {
            ...
            CUDD.InitialiseCUDD(2048, CUDD.CUDD_UNIQUE_SLOTS, CUDD.CUDD_CACHE_SLOTS, 0.1);
        }
        ...
        public void AddLocalVar(string name, int lower, int upper)
        {
            ...
            CUDDNode identity = (numBits == 0) ? CUDD.Constant(1) : CUDD.Matrix.Identity(rowVar, colVar);
        ...
        /// <returns></returns>
        public List<CUDDNode> FilterVariableInRange(List<CUDDNode> transition)
        {
            foreach (var range in this.variableRanges)
            {
                CUDD.Ref(range);
                transition = CUDD.Function.And(transition, range);
            }
            return transition;
        }
        ...
        /// <returns></returns>
        public CUDDNode FilterVariableInRange(CUDDNode state)
        {
            foreach (var range in this.variableRanges)
            {
                CUDD.Ref(range);
                state = CUDD.Function.And(state, range);
            }
            return state;
        }
        ...
        /// </summary>
        public bool IsUnchangedVariable(int index, List<int> updatedVariableIndexes)
        {
        ...
        {
            return this.varList.GetVarName(i);
        }
        ...
        public void PrintAllVariableValue(CUDDNode currentStateDD)
        {
            for (int i = 0; i < this.GetNumberOfVars(); i++)
            {
                string varName = this.GetVarName(i);
                int value = this.GetColVarValue(currentStateDD, varName);
                Debug.WriteLine(varName + " = " + value);
            }
