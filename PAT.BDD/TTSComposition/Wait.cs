...
namespace PAT.Common.Classes.SemanticModels.TTS
{
    public partial class TimeBehaviors
    {
        ...
        {
            result.initExpression = new PrimitiveApplication(PrimitiveApplication.EQUAL, new Variable(state), new IntConstant(0));
        }
        ...
        private static void WaitEncodeTick(string state, int t, Model model, AutomataBDD result)
        {
            ...
            guard = new PrimitiveApplication(PrimitiveApplication.AND, new PrimitiveApplication(PrimitiveApplication.LESS, new Variable(state), new IntConstant(t)),
                        new PrimitiveApplication(PrimitiveApplication.AND, GetTickTransExpression(),
            ...
            guard = new PrimitiveApplication(PrimitiveApplication.AND, new PrimitiveApplication(PrimitiveApplication.OR,
                            new PrimitiveApplication(PrimitiveApplication.EQUAL, new Variable(state), new IntConstant(t)),
                            new PrimitiveApplication(PrimitiveApplication.EQUAL, new Variable(state), new IntConstant(t + 1))),
                                    new PrimitiveApplication(PrimitiveApplication.AND, GetTickTransExpression(),
