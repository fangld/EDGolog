...
namespace PAT.Common.Classes.SemanticModels.LTS.BDD
{
    public partial class AutomataBDD
    {
        ...
        private static void ExternalChoiceSetInit(List<AutomataBDD> choices, AutomataBDD result)
        {
            ...
            {
                result.initExpression = new PrimitiveApplication(PrimitiveApplication.AND, result.initExpression, choices[i].initExpression);
            }

            result.initExpression = new PrimitiveApplication(PrimitiveApplication.AND, result.initExpression,
                                        new PrimitiveApplication(PrimitiveApplication.EQUAL, new Variable(result.newLocalVarName), new IntConstant(-1)));
        }
